...
const getEnvValue = (keys: string[]): string => {
  // 优先取前端编译期注入的环境变量（Vite 推荐 VITE_ 前缀），其次兼容 NEXT_PUBLIC_ / 纯名
...
const ImagePromptGenerator: React.FC = () => {
  ...
  const uploadImageToImgBB = async (file: File): Promise<string> => {
    ...
    const imgbbApiKey = getEnvValue(['VITE_IMGBB_API_KEY', 'NEXT_PUBLIC_IMGBB_API_KEY', 'IMGBB_API_KEY']);
    if (!imgbbApiKey) {
      throw new Error('缺少 ImgBB API Key，请在环境变量中配置 IMGBB_API_KEY（或 VITE_/NEXT_PUBLIC_ 前缀）');
    }
    ...
    try {
      ...
      if (data.success && data.data) {
        // ImgBB 返回多种 URL 格式
        // url: 直接图片链接 (推荐用于API调用)
        // display_url: 展示页面URL (不适用于API)
        // medium.url: 中等尺寸图片URL
        // image.url: 原始图片URL

        const directImageUrl = data.data.url; // 使用直接图片链接
...
请生成专业的产品组合图。`;
  ...
  const optimizePrompt = async () => {
    ...
    try {
      ...
      if (!arkApiKey) {
        throw new Error('缺少 Ark API Key，请在环境变量中配置 ARK_API_KEY（或 VITE_/NEXT_PUBLIC_ 前缀）');
      }
              ...
              <>
                ...
                <div className={styles.formItem}>
                  ...
                  <input
                    ...
                    onChange={(e) => handleChange('productCount', parseInt(e.target.value) || 2)}
                  />
                </div>
